...
namespace GarblingPrize.Submission.GLVCompactOracleLaw

/-!
The protected oracle induces exactly the uniform law on the private GLV
randomness record.  Distinct purpose addresses supply 90 scalar samples,
91 nonzero field randomizers, 91 × 8 chain masks, and 91 × 11 × 253 free
table masks.  Multiplication by the certified generator bijects the scalar
field with G1.  The leading offset is uniquely determined by
`offset[0] = Q - (3 + phi)(recompose offset[1..90])`, and each table's final
mask is its constant minus the sum of the 253 free masks.  These are
bijections onto their constraint fibers, so the complete derived randomness
is uniform, not merely a collection of uniform marginals.
-/
...
def chainMasksFree (masks : GLVProjectiveMap.ChainMasks) :
    Fin 8 → BN254.Fq
  | ⟨0, _⟩ => masks.xLinear
  | ⟨1, _⟩ => masks.xCross
  | ⟨2, _⟩ => masks.xOuter
...
@[simp] theorem chainMasksFromFree_free
    (masks : GLVProjectiveMap.ChainMasks) :
    chainMasksFromFree (chainMasksFree masks) = masks := by
  apply GLVProjectiveMap.ChainMasks.ext <;> rfl

structure FreeRandomness where
  offsets : OffsetTail
  randomizers : Fin 91 → BN254.Fqˣ
  chainMasks : Fin 91 → Fin 8 → BN254.Fq
  tableMasks : Fin 91 → GLVProjectiveMap.TableKind → Fin 253 → BN254.Fq
...
def freeFromOracle (oracle : InternalOracle) : FreeRandomness where
  ...
  tableMasks := fun index kind slot =>
    GLVCompactScheme.tableMaskFree oracle index kind slot
...
theorem freeFromRaw_rawFromOracle (oracle : InternalOracle) :
  ...
  · funext index kind slot
    ...
    apply ZMod.val_injective
    rw [wordEquiv_val]
    rw [show GLVCompactScheme.tableMaskFree oracle index kind slot =
...
end GarblingPrize.Submission.GLVCompactOracleLaw
