import GarblingPrize.Submission.CompactOracleLaw

namespace GarblingPrize.Submission.CompactPrivacy

open GarblingPrize.Protected
open GarblingPrize.Submission.BalancedTernary
open GarblingPrize.Submission.CompactScheme
open GarblingPrize.Submission.CompactOracleLaw
...
def toConcretePoint (point : Profile.G1) : BN254.G1 := point

@[simp] theorem map_toConcretePoint_eq (points : List Profile.G1) :
    points.map toConcretePoint = (show List BN254.G1 from points) := by
...
@[simp] theorem toConcretePoint_input (input : Input) :
    toConcretePoint input.point = inputG1 input := rfl

@[simp] theorem toConcretePoint_neg (point : Profile.G1) :
    toConcretePoint (-point) = -toConcretePoint point := rfl

theorem mapOutputs_toConcrete (offsets : List Profile.G1)
    (digits : List Digit) (point : Profile.G1) :
    (TernaryFullWidth.mapOutputs offsets digits point).map toConcretePoint =
      TernaryFullWidth.mapOutputs (offsets.map toConcretePoint) digits
        (toConcretePoint point) := by
  ...
  | cons offset offsets ih =>
      ...
      | nil => rfl
      | cons digit digits =>
          simp only [TernaryFullWidth.mapOutputs, List.zipWith_cons_cons,
            List.map_cons]
          apply congrArg₂ List.cons
          · rfl
          · simpa only [TernaryFullWidth.mapOutputs] using ih digits
...
theorem mapOutput_preserved (input : Input) (source target : Hidden)
    ...
    TernaryFullWidth.mapOutput (offsetAt (targetOffsets input source target
  ...
  have family := OffsetFamily.selectedOutputs_preserved input source target
    hequal offsets
  have concreteFamily := congrArg (List.map toConcretePoint) family
  rw [mapOutputs_toConcrete, mapOutputs_toConcrete] at concreteFamily
  rw [map_toConcretePoint_eq, map_toConcretePoint_eq,
    toConcretePoint_neg, toConcretePoint_input] at concreteFamily
  have indexed :
      (List.finRange 161).map (fun index =>
          TernaryFullWidth.mapOutput
    ...
    change TernaryFullWidth.mapOutputs
      ...
      TernaryFullWidth.mapOutputs offsets.values
        (OffsetFamily.digits source).values (-inputG1 input)
    exact concreteFamily
  ...
  ProjectiveMapPrivacy.chainMaskEquiv
    ...
    (randomizedCoefficients
      ...
      (targetRandomizer input source target hequal offsets index
        (randomizers index)))
    (input.x.val : Word) (input.y.val : Word) (chainMasks index)
...
theorem tableOutput_preserved (input : Input) (source target : Hidden)
  ...
  · exact input.onCurve
  · exact randomizedCoefficients_xX (offsetAt offsets index)
      (digitAt source index) (randomizers index)
  · exact randomizedCoefficients_xX
      (offsetAt (targetOffsets input source target hequal offsets) index)
      (digitAt target index)
      (targetRandomizer input source target hequal offsets index
        (randomizers index))
  · exact randomizedCoefficients_zXX (offsetAt offsets index)
      (digitAt source index) (randomizers index)
  · exact randomizedCoefficients_zXX
      (offsetAt (targetOffsets input source target hequal offsets) index)
      (digitAt target index)
      (targetRandomizer input source target hequal offsets index
        (randomizers index))
...
def tableKindOfIndex (index : Fin 11) : ProjectiveMap.TableKind :=
...
theorem tableKindOfIndex_index (kind : ProjectiveMap.TableKind) :
    tableKindOfIndex ⟨kind.index, by cases kind <;> decide⟩ = kind := by
  cases kind <;> rfl

def tableOwner (purpose : Purpose) :
    Option (Fin 161 × ProjectiveMap.TableKind) :=
  if hbound : purpose < 11 * 161 then
    let index : Fin 161 := ⟨purpose / 11, by
...
theorem tableOwner_encoded (index : Fin 161) (kindIndex : Fin 11) :
    tableOwner (11 * index.val + kindIndex.val) =
      some (index, tableKindOfIndex kindIndex) := by
  have hbound : 11 * index.val + kindIndex.val < 11 * 161 := by
    have hi := index.isLt
  ...
  have hindexFin :
      (⟨(11 * index.val + kindIndex.val) / 11, by
        rw [hdiv]
        exact index.isLt⟩ : Fin 161) = index := Fin.ext hdiv
  have hkindFin :
      (⟨(11 * index.val + kindIndex.val) % 11, by
        rw [hmod]
...
@[simp] theorem tableOwner_purpose (index : Fin 161)
    (kind : ProjectiveMap.TableKind) :
    tableOwner (ProjectiveMap.purpose index.val kind) = some (index, kind) := by
  unfold ProjectiveMap.purpose
  rw [tableOwner_encoded index ⟨kind.index, by cases kind <;> decide⟩]
  rw [tableKindOfIndex_index]

def kindUsesX : ProjectiveMap.TableKind → Bool
  | .shared | .xXY | .yCubic | .yQuadratic | .yLinear |
      .zSquare | .zCross | .zLinear => true
  | .xYY | .xCorrection | .zCorrection => false

def tableWire (kind : ProjectiveMap.TableKind)
    (index : Fin IdealAffineTable.tableWidth) : BitIndex :=
  if kindUsesX kind then xWire index else yWire index

theorem inputBit_tableWire (kind : ProjectiveMap.TableKind) (input : Input)
...
abbrev ArtifactPads :=
  BitIndex → Bool → Fin (11 * 161) → IdealAffineTable.WordBytes
...
def expandArtifactPads (pads : ArtifactPads) : LabelPairs :=
  fun wire bit purpose =>
    if hpurpose : purpose < 11 * 161 then
...
theorem garble_expandArtifactPads_artifactPads (hidden : Hidden)
  ...
  funext pair
  have hpurpose : ProjectiveMap.purpose mapIndex.val kind < 11 * 161 := by
    have hmap := mapIndex.isLt
    have hkind : kind.index < 11 := by cases kind <;> decide
    unfold ProjectiveMap.purpose
    have hmap' : mapIndex.val ≤ 160 := by omega
    have hkind' : kind.index ≤ 10 := by omega
    calc
      11 * mapIndex.val + kind.index ≤ 11 * 160 + 10 :=
        Nat.add_le_add (Nat.mul_le_mul_left 11 hmap') hkind'
      _ < 11 * 161 := by decide
  unfold garble ProjectiveMap.garble IdealAffineTable.garble
  ...
  cases kind <;>
    simp [ProjectiveMap.pairsFor, xPairs, yPairs, hpurpose]

def tableIndex (kind : ProjectiveMap.TableKind) (wire : BitIndex) :
...
@[simp] theorem tableIndex_tableWire (kind : ProjectiveMap.TableKind)
    (index : Fin IdealAffineTable.tableWidth) :
...
@[simp] theorem payloadAt_tableWire (hidden : Hidden)
    (randomness : Randomness hidden) (mapIndex : Fin 161)
    (kind : ProjectiveMap.TableKind) (index : Fin IdealAffineTable.tableWidth)
    (bit : Bool) :
    payloadAt hidden randomness (tableWire kind index) bit
        (ProjectiveMap.purpose mapIndex.val kind) =
      IdealAffineTablePrivacy.payload
        ((mapHidden hidden randomness.offsets randomness.randomizers
...
theorem targetTableMask_eq (input : Input) (source target : Hidden)
    (hequal : reference Profile source input = reference Profile target input)
    (randomness : Randomness source) (index : Fin 161)
    (kind : ProjectiveMap.TableKind) :
...
theorem tableCipher_preserved (input : Input) (source target : Hidden)
    ...
    (mapIndex : Fin 161) (kind : ProjectiveMap.TableKind)
    ...
    Bytes.xor
      ...
      Bytes.xor
        ...
        (pairs (tableWire kind index) bit
          (ProjectiveMap.purpose mapIndex.val kind)) := by
  dsimp only
  let sourceParams :=
    (mapHidden source randomness.offsets randomness.randomizers
      randomness.chainMasks mapIndex).params kind
...
theorem tableGarble_preserved (input : Input) (source target : Hidden)
    ...
    let targetPairs := transformPairs input source target randomness transformed
      pairs
    IdealAffineTable.garble (ProjectiveMap.purpose mapIndex.val kind)
        (ProjectiveMap.pairsFor (xPairs targetPairs) (yPairs targetPairs) kind)
        ((mapHidden target transformed.offsets transformed.randomizers
          transformed.chainMasks mapIndex).params kind)
        ⟨transformed.tableMasks mapIndex kind,
          transformed.tableMasks_sum mapIndex kind⟩ =
      IdealAffineTable.garble (ProjectiveMap.purpose mapIndex.val kind)
        (ProjectiveMap.pairsFor (xPairs pairs) (yPairs pairs) kind)
        ((mapHidden source randomness.offsets randomness.randomizers
          randomness.chainMasks mapIndex).params kind)
        ⟨randomness.tableMasks mapIndex kind,
  ...
  unfold IdealAffineTable.ciphertextForSlot
  cases kind <;>
    simpa only [ProjectiveMap.pairsFor, xPairs, yPairs, tableWire, kindUsesX,
...
theorem mapGarble_preserved (input : Input) (source target : Hidden)
    ...
    let targetPairs := transformPairs input source target randomness transformed
      pairs
    ProjectiveMap.garble mapIndex.val (xPairs targetPairs) (yPairs targetPairs)
        (mapHidden target transformed.offsets transformed.randomizers
          transformed.chainMasks mapIndex)
        (fun kind => ⟨transformed.tableMasks mapIndex kind,
          transformed.tableMasks_sum mapIndex kind⟩) =
      ProjectiveMap.garble mapIndex.val (xPairs pairs) (yPairs pairs)
        (mapHidden source randomness.offsets randomness.randomizers
...
theorem productArtifactBytes_measurable (hidden : Hidden) :
    Measurable (fun state : Randomness hidden × LabelPairs =>
      FamilyArtifact.encode (garble hidden state.1 state.2)) := by
  ...
  funext state
  exact congrArg FamilyArtifact.encode
...
noncomputable def productView (hidden : Hidden) (input : Input)
  ...
  (PublicView.measurableEquiv Profile).symm
    ((FamilyArtifact.encode (garble hidden state.1 state.2),
...
theorem productView_productChange (input : Input) (source target : Hidden)
  ...
  · exact congrArg
      (fun artifact : FamilyArtifact.Artifact 161 =>
        FamilyArtifact.encode artifact)
      hartifact
  · exact hlabels
...
end GarblingPrize.Submission.CompactPrivacy
