...
namespace G1Release.Submission.LamportGuessing
...
theorem complement_card (active : Label) : Nat.card (Complement active) = 2^256-1 := by
  ...
  simp only [Fintype.card_unique, ← Nat.card_eq_fintype_card, FiniteProbability.label_card]
...
end G1Release.Submission.LamportGuessing
