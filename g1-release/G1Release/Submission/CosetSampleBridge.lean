...
namespace G1Release.Submission.CosetSampleBridge
open SecretRelease G1Release.Protected CosetSampler CosetGameLaw CosetSamplerMeasure
...
def state (hidden : Hidden) (ω : ROM.Sample (CosetScheme.scheme n sampler)) : RealState hidden :=
  (ω.1, sampler ω.2.2.1 hidden, ω.2.2.2)

theorem preserving (hidden : Hidden) :
    MeasurePreserving (state n sampler hidden) (ROM.law (CosetScheme.scheme n sampler))
...
theorem outcome_eq (hidden : Hidden) (input : Input) (adversary : View challenge → Program α)
    (ω : ROM.Sample (CosetScheme.scheme n sampler)) :
    ROM.run (ROM.hash ω.2.2.2) (adversary (ROM.view (CosetScheme.scheme n sampler) hidden input ω)) =
...
theorem winEvent_eq (hidden : Hidden) (input : Input)
    ...
    ROM.winEvent (CosetScheme.scheme n sampler) hidden input adversary =
...
theorem distinguishEvent_eq (hidden : Hidden) (input : Input)
    ...
    ROM.distinguishEvent (CosetScheme.scheme n sampler) hidden input adversary =
      state n sampler hidden ⁻¹' realDistinguish hidden input adversary := rfl

theorem win_measurable (hidden : Hidden) (input : Input)
    (adversary : View challenge → Program challenge.Claim) :
    MeasurableSet (ROM.winEvent (CosetScheme.scheme n sampler) hidden input adversary) := by
  rw [winEvent_eq]
...
theorem distinguish_measurable (hidden : Hidden) (input : Input)
    (adversary : View challenge → Program Bool) :
    MeasurableSet (ROM.distinguishEvent (CosetScheme.scheme n sampler) hidden input adversary) := by
...
theorem distinguish_probability_eq (hidden : Hidden) (input : Input)
    (adversary : View challenge → Program Bool) :
    ROM.law (CosetScheme.scheme n sampler) (ROM.distinguishEvent (CosetScheme.scheme n sampler) hidden input adversary) =
...
end G1Release.Submission.CosetSampleBridge
