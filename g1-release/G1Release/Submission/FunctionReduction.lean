import G1Release.Submission.GameHops
import G1Release.Submission.IdealGameCoupling

set_option maxRecDepth 4096
set_option maxHeartbeats 300000

namespace G1Release.Submission.FunctionReduction
open SecretRelease G1Release.Protected Randomized RealIdeal GameLaw SamplerMeasure
...
theorem probability_le (source target : Hidden) (input : Input)
  ...
  calc
    ...
    _ ≤ (extendedLaw source (idealLaw source) (idealDistinguish source input adversary) + guess) + error :=
      add_le_add (GameHops.real_le_ideal source input (idealLaw source) adversary q ha) le_rfl
    _ = (extendedLaw target (idealLaw target) (idealDistinguish target input adversary) + guess) + error := by
      rw [IdealGameCoupling.probability_eq source target input hequal href adversary]
    _ ≤ ((realLaw target (idealLaw target) (realDistinguish target input adversary) + guess) + guess) + error :=
      add_le_add (add_le_add (GameHops.ideal_le_real target input (idealLaw target) adversary q ha) le_rfl) le_rfl
...
end G1Release.Submission.FunctionReduction
