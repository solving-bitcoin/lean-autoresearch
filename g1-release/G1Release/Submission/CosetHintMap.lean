...
namespace G1Release.Submission.CosetHintMap
...
def garble (mapIndex : Nat) (xPairs yPairs : RowIndex → Bool → PadFamily)
...
def evaluateTable (mapIndex : Nat) (artifact : Artifact) (kind : TableKind)
    (xBits yBits : RowIndex → Bool) (xLabels yLabels : RowIndex → PadFamily) : Option Word :=
...
theorem evaluateTable_garble (mapIndex : Nat) (xPairs yPairs : RowIndex → Bool → PadFamily)
    ...
    evaluateTable mapIndex (garble mapIndex xPairs yPairs base state coins) kind xBits yBits
      ...
      some (openedWords (opened base state (C (HintPayload.decodeBits xBits))
        (C (HintPayload.decodeBits yBits))) kind) := by
  rw [← params_opened]
...
def evaluate (mapIndex : Nat) (artifact : Artifact) (xBits yBits : RowIndex → Bool)
    (xLabels yLabels : RowIndex → PadFamily) : Option (Opened K) := do
...
theorem evaluate_garble (mapIndex : Nat) (xPairs yPairs : RowIndex → Bool → PadFamily)
    ...
    evaluate mapIndex (garble mapIndex xPairs yPairs base state coins) xBits yBits
      (fun i => xPairs i (xBits i)) (fun i => yPairs i (yBits i)) =
      some (opened base state (C (HintPayload.decodeBits xBits))
        (C (HintPayload.decodeBits yBits))) := by
...
end G1Release.Submission.CosetHintMap
