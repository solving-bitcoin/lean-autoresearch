...
def xorEquiv (mask : Label) : Label ≃ Label where
  toFun value := Bytes.xor mask value
  invFun value := Bytes.xor mask value
  left_inv := Bytes.xor_cancel_left mask
  right_inv := Bytes.xor_cancel_left mask

theorem xor_preserving {D : Type*} (mask : D → Label) :
    MeasurePreserving (fun oracle : D → Label => fun d => Bytes.xor (mask d) (oracle d))
      (law D) (law D) := by
  refine ⟨measurable_pi_lambda _ fun d => (Measurable.of_discrete : Measurable (fun value : Label => Bytes.xor (mask d) value)).comp (measurable_pi_apply d), ?_⟩
  rw [law, Measure.infinitePi_map_pi _ (fun _ => Measurable.of_discrete)]
  congr 1
  funext d
  exact (FiniteProbability.uniform_equiv (xorEquiv (mask d))).map_eq

theorem masked_reindex_preserving {D E : Type*} (f : D → E)
    (hf : Function.Injective f) (mask : D → Label) :
    MeasurePreserving (fun oracle : E → Label => fun d => Bytes.xor (mask d) (oracle (f d)))
      (law E) (law D) :=
...
noncomputable def programmedMask {D C : Type*} (address : C → D) (payload : C → Label)
    (query : D) : Label := by
  letI : Decidable (∃ c, address c = query) := Classical.propDecidable _
  exact if h : ∃ c, address c = query then payload (Classical.choose h) else Bytes.zero 32

noncomputable def program {D C : Type*} (address : C → D) (payload : C → Label)
    (oracle : D ⊕ C → Label) (query : D) : Label :=
  Bytes.xor (programmedMask address payload query) (oracle (programmedIndex address query))
...
theorem program_at {D C : Type*} (address : C → D) (ha : Function.Injective address)
    ...
    program address payload oracle (address c) = Bytes.xor (payload c) (oracle (.inr c)) := by
  classical
...
end G1Release.Submission.OracleLaw
