import G1Release.Submission.DutyFreeGameHops
import G1Release.Submission.DutyFreeIdealGameCoupling
import G1Release.Submission.CosetSamplerMeasure

set_option maxRecDepth 4096
set_option maxHeartbeats 300000

namespace G1Release.Submission.DutyFreeFunctionReduction

local instance : MeasurableSpace DutyFreeSlots.Keys := ⊤
open SecretRelease G1Release.Protected CosetSampler DutyFreeRealIdeal DutyFreeGameLaw CosetSamplerMeasure
...
theorem realLaw_close (hidden : Hidden) :
  ...
  · exact EventBounds.prod_left
      ...
      (uniformOn (Set.univ : Set KeyPair))
  · exact EventBounds.prod_left
...
theorem probability_le (source target : Hidden) (input : Input)
    ...
    realLaw source (samplerLaw coinBytes sample source) (realDistinguish source input adversary) ≤
      realLaw target (samplerLaw coinBytes sample target) (realDistinguish target input adversary) +
        (2 * ((q : ENNReal) / (2^256-1 : Nat)) + 2 * ENNReal.ofReal samplingError + 1 / 2^240) := by
  ...
  calc
    ...
    _ ≤ (extendedLaw source (idealLaw source) (idealDistinguish source input adversary) + guess) + error :=
      add_le_add (DutyFreeGameHops.real_le_ideal source input (idealLaw source) adversary q ha) le_rfl
    _ ≤ ((extendedLaw target (idealLaw target) (idealDistinguish target input adversary) + 1 / 2^240) + guess) + error :=
      add_le_add (add_le_add (DutyFreeIdealGameCoupling.probability_le source target input hequal href adversary) le_rfl) le_rfl
    _ ≤ (((realLaw target (idealLaw target) (realDistinguish target input adversary) + guess) + 1 / 2^240) + guess) + error :=
      add_le_add (add_le_add (add_le_add
        (DutyFreeGameHops.ideal_le_real target input (idealLaw target) adversary q ha) le_rfl) le_rfl) le_rfl
    _ ≤ ((((realLaw target (samplerLaw coinBytes sample target) (realDistinguish target input adversary) + error) + guess) + 1 / 2^240) + guess) + error :=
      add_le_add (add_le_add (add_le_add (add_le_add ((realLaw_close target).2 _
        (realDistinguish_measurable target input adversary)) le_rfl) le_rfl) le_rfl) le_rfl
    _ = _ := by dsimp only [error,guess]; rw [two_mul, two_mul]; ac_rfl

end G1Release.Submission.DutyFreeFunctionReduction
