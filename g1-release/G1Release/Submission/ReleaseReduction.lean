...
import G1Release.Submission.KeyInverse

set_option maxRecDepth 4096
set_option maxHeartbeats 800000

namespace G1Release.Submission.ReleaseReduction
...
theorem realOutcome_programmed (hidden : Hidden) (input : Input)
    ...
    realOutcome hidden input adversary (programState hidden input (keys,random,oracle)) =
      ROM.run (ROM.hash (programmed hidden random keys input oracle))
        (adversary (GameViews.ideal hidden random input (selected keys input) oracle)) := by
  unfold realOutcome programState
  rw [GameViews.real_programmed]
...
theorem conditioned_subset (hidden : Hidden) (random : Randomness hidden) (input : Input)
    ...
    {ranks : Fin 512 → Rank |
      programState hidden input (KeyInverse.keys input active ranks,random,oracle) ∈
        realWin hidden input adversary} ⊆
    hiddenFromRanks active ⁻¹' guessesEvent active
      (guesses (ROM.hash (baseOracle oracle)) (adversary (GameViews.ideal hidden random input active oracle))) := by
  intro ranks hw
  let keys := KeyInverse.keys input active ranks
  let program := adversary (GameViews.ideal hidden random input active oracle)
  have hwin :
    ...
    simp only [realWin, Set.mem_setOf_eq, realOutcome_programmed, KeyInverse.selected_keys] at hw
    exact hw
  ...
  exact (KeyInverse.opposite_keys input active ranks guess.1).symm.trans hsecret
...
theorem conditioned_probability_le (hidden : Hidden) (random : Randomness hidden) (input : Input)
    ...
    uniformOn (Set.univ : Set (Fin 512 → Rank))
      {ranks | programState hidden input (KeyInverse.keys input active ranks,random,oracle) ∈
  ...
  have he : MeasurableSet (guessesEvent active
      (guesses (ROM.hash (baseOracle oracle)) (adversary (GameViews.ideal hidden random input active oracle)))) :=
...
end G1Release.Submission.ReleaseReduction
