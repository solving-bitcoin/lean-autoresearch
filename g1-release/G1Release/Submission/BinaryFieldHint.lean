...
namespace G1Release.Submission.BinaryFieldHint

/-!
An exact randomized conversion from a binary pad to a prime-field mask with
one public hint bit, ported from codex/test-ci. Its finite fiber theorem
supports the conditional row transport used by this submission.
...
For BN254 choose M=2^255. A false row decodes its mask from 255 oracle
answer bits and the public hint. A true row stores a 254-bit ciphertext.
The byte-aligned row uses 32 bytes, and one correction word adds 32 bytes
per table. The eight-table, 91-map construction therefore has size
91*8*255*32=5,940,480 bytes. Its finite-ROM certificate is a separate proof.
...
end G1Release.Submission.BinaryFieldHint
