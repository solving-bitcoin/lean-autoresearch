...
namespace G1Release.Submission.LamportLaw
open SecretRelease MeasureTheory ProbabilityTheory LamportGuessing
...
theorem keysEquiv_preserving {ι : Type*} [Fintype ι] (bits : ι → Bool) :
  ...
  exact FiniteProbability.uniform_equiv (keysEquiv bits)
...
theorem hiddenFromRanks_preserving {ι : Type*} [Fintype ι] (active : ι → Label) :
  ...
  congr 1
  funext i
  exact (FiniteProbability.uniform_equiv (complementEquiv (active i)).symm).map_eq
...
end G1Release.Submission.LamportLaw
