...
def payload (params : Params) (index : RowIndex) (state : RowState) : WordBytes :=
  HintPayload.encodeWord (HintPayload.share params (stateMask state) index true)

def rowView (params : Params) (index : RowIndex) (state : RowState) : WordBytes :=
  pack (HintPayload.encrypt (payload params index state) state.2) (stateHint state)

def translatePadEquiv (oldPayload newPayload : WordBytes) : Equiv.Perm WordBytes where
  toFun := HintPayloadPrivacy.translatePad oldPayload newPayload
  invFun := HintPayloadPrivacy.translatePad newPayload oldPayload
  left_inv := HintPayloadPrivacy.translatePad_source_target_source oldPayload newPayload
  right_inv := HintPayloadPrivacy.translatePad_source_target_source newPayload oldPayload

def falseTransport (source target : Params) (index : RowIndex) : Equiv.Perm RowState :=
  Equiv.prodCongrRight fun padCoin =>
    translatePadEquiv
      (HintPayload.encodeWord (HintPayload.share source
        (maskFromPad padCoin.1 (padHint padCoin.1 padCoin.2)) index true))
      (HintPayload.encodeWord (HintPayload.share target
        (maskFromPad padCoin.1 (padHint padCoin.1 padCoin.2)) index true))
...
noncomputable def rowTransport (source target : Params) (index : RowIndex)
  ...
  if selected then
    Equiv.prodCongr (padCoinTransport
      (HintPayload.weight index * (source.coefficient - target.coefficient))) (Equiv.refl _)
...
theorem rowTransport_false_apply (source target : Params) (index : RowIndex) (state : RowState) :
    rowTransport source target index false state =
      (state.1, HintPayloadPrivacy.translatePad (payload source index state)
...
theorem rowTransport_true_apply (source target : Params) (index : RowIndex) (state : RowState) :
    rowTransport source target index true state =
      (padCoinTransport (HintPayload.weight index *
...
theorem rowTransport_mask (source target : Params) (index : RowIndex)
    ...
    stateMask (rowTransport source target index selected state) = stateMask state +
      HintPayload.weight index * (source.coefficient - target.coefficient) *
        HintPayload.bitWord selected := by
  cases selected
  · rw [rowTransport_false_apply]
    simp [stateMask, stateHint, HintPayload.bitWord]
  · rw [rowTransport_true_apply]
    simpa only [stateMask, stateHint, HintPayload.bitWord, ↓reduceIte, mul_one] using
      padCoinTransport_mask
        (HintPayload.weight index * (source.coefficient - target.coefficient)) state.1
...
theorem rowTransport_row (source target : Params) (index : RowIndex)
  ...
  · have hpayload : payload target index (rowTransport source target index true state) =
      ...
      unfold payload
      rw [HintPayload.share_eq_weight, HintPayload.share_eq_weight, rowTransport_mask]
      apply congrArg HintPayload.encodeWord
      simp only [HintPayload.bitWord, ↓reduceIte, mul_one]
      ring
    unfold rowView
...
theorem rowTransport_preserves_uniform (source target : Params) (index : RowIndex)
  ...
  exact FiniteProbability.uniform_equiv _

def tableFromState (params : Params) (states : RowIndex → RowState) : Table :=
  Table.ofWords (Fin.cases
    (HintPayload.encodeWord (params.constant - ∑ i, stateMask (states i)))
    (fun index => rowView params index (states index)))

theorem garble_eq_tableFromState (purpose : Purpose) (pairs : RowIndex → Bool → PadFamily)
...
theorem tableFromState_transport (source target : Params) (bits : RowIndex → Bool)
    (houtput : source.coefficient * HintPayload.decodeBits bits + source.constant =
      target.coefficient * HintPayload.decodeBits bits + target.constant)
  ...
  have hsum : (∑ i, stateMask (tableTransport source target bits states i)) =
      (∑ i, stateMask (states i)) + (source.coefficient - target.coefficient) *
        HintPayload.decodeBits bits := by
  ...
  · change HintPayload.encodeWord (target.constant - ∑ i, stateMask
      (tableTransport source target bits states i)) = _
    rw [hsum]
    apply congrArg HintPayload.encodeWord
    change target.constant - ((∑ i, stateMask (states i)) +
      (source.coefficient - target.coefficient) * HintPayload.decodeBits bits) =
...
end G1Release.Submission.HintAffineTablePrivacy
