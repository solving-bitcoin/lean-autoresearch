...
namespace G1Release.Submission.CosetQueryGuessing
open SecretRelease G1Release.Protected CosetSlots CosetSampler CosetRealIdeal
open OracleComp MeasureTheory

def encoded (value : Slot × Label) : List (Fin 256) :=
  OracleAddress.query value.2 (CosetSlots.purpose value.1) value.1.2.2.val

theorem encoded_injective : Function.Injective encoded := by
  intro a b h
  have hc := OracleAddress.query_injective (CosetSlots.purpose_lt a.1)
    (CosetSlots.purpose_lt b.1) (by omega : a.1.2.2.val < 65536)
      (by omega : b.1.2.2.val < 65536) h
  exact Prod.ext (CosetSlots.slot_eq hc.2.1 (Fin.ext hc.2.2)) hc.1
...
noncomputable def listGuess (query : List (Fin 256)) : Option (Fin 512 × Label) := by
  ...
  exact if h : ∃ value, encoded value = query then
    some (CosetSlots.wire (Classical.choose h).1, (Classical.choose h).2) else none

theorem listGuess_encoded (value : Slot × Label) :
    listGuess (encoded value) = some (CosetSlots.wire value.1,value.2) := by
  classical
...
theorem oppositeKeys_slot (keys : Fin 512 → Pair) (input : Input) (slot : Slot) :
    (oppositeKeys keys input (CosetSlots.wire slot)).val =
      (keys (CosetSlots.wire slot)).get (!(CosetSlots.bit input slot)) := by
  dsimp only [oppositeKeys, CosetSlots.wire, CosetSlots.bit]
...
end G1Release.Submission.CosetQueryGuessing
