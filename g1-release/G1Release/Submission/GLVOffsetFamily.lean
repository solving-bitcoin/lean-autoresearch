...
def digits (hidden : Hidden) : List Digit :=
  EisensteinRadix.digits 91 (EisensteinRadix.reduceScalar hidden.2.val)
...
def mapAt (hidden : Hidden) (input : Input) : Point :=
  qPoint hidden + hidden.2.val • inputPoint input
...
theorem translateValues_total (input : Input) (source target : Hidden)
  ...
  rw [show EisensteinFullWidth.recompose
        ...
        target.2.val • (-inputPoint input) by
      exact EisensteinFullWidth.recompose_scalar target.2 (-inputPoint input),
    show EisensteinFullWidth.recompose
        ((digits source).map fun digit =>
          EisensteinFullWidth.digitTerm digit (-inputPoint input)) =
        source.2.val • (-inputPoint input) by
      exact EisensteinFullWidth.recompose_scalar source.2 (-inputPoint input)]
  ...
  calc
    qPoint source +
          (-(target.2.val • inputPoint input) -
            -(source.2.val • inputPoint input)) =
...
end G1Release.Submission.GLVOffsetFamily
