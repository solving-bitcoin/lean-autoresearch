import G1Release.Submission.CosetReleaseReduction
import G1Release.Submission.CosetTraceMeasurable

set_option maxRecDepth 4096
set_option maxHeartbeats 800000

namespace G1Release.Submission.CosetBadQueries
...
def event (hidden : Hidden) (input : Input) (adversary : View challenge → Program α) :
    Set (ExtendedState hidden) := {state |
      ∃ query ∈ ROMTrace.queries (ROM.hash (baseOracle state.2.2))
        ...
        ∃ slot, CosetSlots.inactive state.1 input slot = query.data.toList.map UInt8.toFin}
...
theorem measurable (hidden : Hidden) (input : Input) (adversary : View challenge → Program α) :
  ...
  let hits (keys : Keys) (queriesFound : List ByteArray) : Bool :=
    decide (∃ query ∈ queriesFound, ∃ slot, CosetSlots.inactive keys input slot =
  ...
  have hm : Measurable (fun state : ExtendedState hidden => hits state.1
      (ROMTrace.queries (ROM.hash (baseOracle state.2.2))
        (adversary (CosetGameViews.ideal hidden state.2.1 input (selected state.1 input) state.2.2)))) := by
    apply measurable_from_prod_countable_right
    intro keys
    apply measurable_from_prod_countable_right
    intro random
    have hs : Measurable (hits keys) := Measurable.of_discrete
    have hr := CosetTraceMeasurable.ideal_trace_measurable hidden random input (selected keys input) adversary
    have hc := hs.comp hr
    exact hc
...
theorem hit_implies_guesses (keys : Keys) (input : Input) (hash : Hash) (program : Program α)
    (hh : ∃ query ∈ ROMTrace.queries hash program,
      ∃ slot, CosetSlots.inactive keys input slot = query.data.toList.map UInt8.toFin) :
    oppositeKeys keys input ∈ guessesEvent (selected keys input) (queryGuesses hash program) := by
  obtain ⟨query,hquery,slot,hslot⟩ := hh
  let g : Fin 512 × Label :=
    (CosetSlots.wire slot, (keys (CosetSlots.wire slot)).get (!(CosetSlots.bit input slot)))
...
theorem conditioned_subset (hidden : Hidden) (random : Randomness hidden) (input : Input)
    ...
    {ranks : Fin 512 → Rank | (CosetKeyInverse.keys input active ranks,random,oracle) ∈ event hidden input adversary} ⊆
      hiddenFromRanks active ⁻¹' guessesEvent active
        (queryGuesses (ROM.hash (baseOracle oracle)) (adversary (CosetGameViews.ideal hidden random input active oracle))) := by
  intro ranks hr
  simp only [event, Set.mem_setOf_eq, CosetKeyInverse.selected_keys] at hr
  have hh := hit_implies_guesses (CosetKeyInverse.keys input active ranks) input
    (ROM.hash (baseOracle oracle)) (adversary (CosetGameViews.ideal hidden random input active oracle)) hr
  obtain ⟨guess,hguess,hsecret⟩ := hh
  exact ⟨guess,hguess,(CosetKeyInverse.opposite_keys input active ranks guess.1).symm.trans hsecret⟩
...
theorem conditioned_probability_le (hidden : Hidden) (random : Randomness hidden) (input : Input)
    ...
    uniformOn (Set.univ : Set (Fin 512 → Rank))
      {ranks | (CosetKeyInverse.keys input active ranks,random,oracle) ∈ event hidden input adversary} ≤
        (q : ENNReal) / (2^256-1 : Nat) := by
  apply (measure_mono (conditioned_subset hidden random input oracle active adversary)).trans
  let gs := queryGuesses (ROM.hash (baseOracle oracle)) (adversary (CosetGameViews.ideal hidden random input active oracle))
...
theorem outcomes_agree (hidden : Hidden) (input : Input) (adversary : View challenge → Program α)
  ...
  rw [CosetReleaseReduction.realOutcome_programmed]
  exact (run_eq_without_hit hidden state.2.1 state.1 input state.2.2
    (adversary (CosetGameViews.ideal hidden state.2.1 input (selected state.1 input) state.2.2)) h).symm

end G1Release.Submission.CosetBadQueries
