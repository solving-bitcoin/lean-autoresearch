import G1Release.Submission.DREFamily

namespace G1Release.Submission.HomogeneousRCB
...
theorem xRow_evaluation
    (c : F) (hidden point : Point F)
    (selector : Bool) (sign : Sign) :
    G1Release.Submission.DRE.rowEvaluation
        (rowWeights (F := F) .x)
        (rowBasis point .x)
        (rowCoefficients c hidden selector sign .x) =
      selectedX c hidden point selector sign := by
  simp only [G1Release.Submission.DRE.rowEvaluation, G1Release.Submission.DRE.weightedSum,
    G1Release.Submission.DRE.monomialUnmasked, rowWeights, one_mul]
  change
    (∑ index : Fin 6,
...
end G1Release.Submission.HomogeneousRCB
