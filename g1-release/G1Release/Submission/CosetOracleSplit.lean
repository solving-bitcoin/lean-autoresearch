...
namespace G1Release.Submission.CosetOracleSplit
open SecretRelease MeasureTheory ProbabilityTheory CosetSlots CosetRealIdeal

abbrev Aux := CosetIdealView.InactivePads

noncomputable def shuffle {X : Type*} [MeasurableSpace X] :
    (X × ExtendedOracle) ≃ᵐ (ROM.Oracle × X × Aux) where
  toFun s := (baseOracle s.2,s.1,fakePads s.2)
...
end G1Release.Submission.CosetOracleSplit
