import G1Release.Submission.CosetBadQueries
import G1Release.Submission.EventBounds

set_option maxRecDepth 4096
set_option maxHeartbeats 300000

namespace G1Release.Submission.CosetGameHops
...
theorem real_le_ideal (hidden : Hidden) (input : Input)
  ...
  apply (EventBounds.event_le_add_bad (extendedLaw hidden randomLaw) _ _
    (CosetBadQueries.event hidden input adversary) ?_).trans
      (add_le_add le_rfl (CosetBadQueries.probability_le hidden input randomLaw adversary q ha))
  intro state hs hb
  ...
  rw [← CosetBadQueries.outcomes_agree hidden input adversary state hb]
  exact hs
...
end G1Release.Submission.CosetGameHops
