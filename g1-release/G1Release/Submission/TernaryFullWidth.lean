import G1Release.Submission.BalancedTernary

namespace G1Release.Submission.TernaryFullWidth
...
theorem signedRecompositionList_eq (digits : List Digit) (point : G) :
    signedRecompositionList digits point =
      G1Release.Submission.BalancedTernary.Digits.decodeList digits • point := by
  induction digits with
  | nil => simp [signedRecompositionList,
      G1Release.Submission.BalancedTernary.Digits.decodeList]
  | cons digit tail ih =>
      ...
      change
        digit.value • point + 3 • signedRecompositionList tail point =
          (digit.value + 3 *
            G1Release.Submission.BalancedTernary.Digits.decodeList tail) • point
      rw [ih, add_zsmul, mul_zsmul]
      exact congrArg (digit.value • point + ·)
        (ofNat_zsmul
          (G1Release.Submission.BalancedTernary.Digits.decodeList tail • point) 3).symm
          ...
          change
            mapOutput offset digit point +
              ...
              offsetTotal (offset :: offsets) -
                (digit.value + 3 *
                    G1Release.Submission.BalancedTernary.Digits.decodeList digits) •
                  point
          rw [ih digits htail]
...
theorem offsetTotal_translateOffsets (offsets : List G)
    ...
    offsetTotal (translateOffsets offsets source target point) =
      offsetTotal offsets +
        (G1Release.Submission.BalancedTernary.Digits.decodeList target -
          G1Release.Submission.BalancedTernary.Digits.decodeList source) • point := by
  induction offsets generalizing source target with
  | nil =>
      cases source with
      | cons source sources => simp at hsource
...
end G1Release.Submission.TernaryFullWidth
