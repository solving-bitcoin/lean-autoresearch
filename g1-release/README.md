...
canonical result bytes. The submission owns the injectivity and equal-leakage
lemmas connecting these bytes to equality of the mathematical point. In particular, at a fixed input, `(Q,r)` has the same leakage
...
The checked-in entry carries a complete certificate at **28,789,376 bytes**:
`161 × 11 × 254 × 2 × 32`. Its 161 balanced-ternary projective maps use random
offsets with a prescribed Horner sum, nonzero scales and independent masks.
The ROM proofs include the bias of sampling these values from finite coins.
...
`Tests`, outside the challenge target's import graph.
...
certified candidate supports the complete pipeline. Uncertified candidates may
also run while their proofs are being developed; their accepted score remains null. See the [shared protocol](../secret-release/ARCHITECTURE.md).
