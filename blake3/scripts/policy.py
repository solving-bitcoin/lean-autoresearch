...
def check_source(submission):
    source_policy.VERIFIER_OWNED_NAMESPACES=(('Blake3Prize','Protected'),('Blake3Prize','Tests'),('Blake3Prize','Migration'),('SecretRelease',))
    source_policy.allowed_import=lambda m: m in {'Blake3Prize.Protected.Target','SecretRelease','SecretRelease.Encoding','SecretRelease.Profiles','SecretRelease.Runtime','SecretRelease.Simulation','SecretRelease.Examples'} or m.startswith('VCVio.OracleComp.QueryTracking.') or m.startswith('VCVio.OracleComp.SimSemantics.') or m.startswith('VCVio.EvalDist.') or m.startswith('Blake3Prize.Submission.') or m=='Mathlib' or m.startswith('Mathlib.')
