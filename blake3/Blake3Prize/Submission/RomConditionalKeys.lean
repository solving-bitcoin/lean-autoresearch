...
namespace Blake3Prize.Submission.RomConditionalKeys
open SecretRelease MeasureTheory ProbabilityTheory RomLamportLaw
...
theorem bound {ι X : Type*} [Fintype ι] [MeasurableSpace X]
  ...
  have hk : MeasurePreserving (keysEquiv bits).symm
    ...
    exact RomFiniteProbability.uniform_equiv (keysEquiv bits).symm
...
end Blake3Prize.Submission.RomConditionalKeys
